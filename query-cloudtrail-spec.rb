...
describe 'query-cloudtrail' do
  before(:each) do
    # silence script STDOUT output
  ...
  describe '#events_for_bucket' do
    ...
    it 'returns all cloudtrail events for a bucket' do
      expect(events_for_bucket(client, 'foo').map {|e| e[:event_id]}).to eq(['one', 'two'])
    end

    it 'uses UTC timestamps' do
      expect(events_for_bucket(client, 'foo')[0][:event_time]).to be_utc
    end

    it 'sleeps to avoid rate limiting' do
      expect_any_instance_of(Object).to receive(:sleep).and_return(:sleep)
      events_for_bucket(client, 'foo')
    end
  end

  describe '#add_bucket_events(list, buckets, cloudtrail_client)' do
    it 'adds the events to the list' do
      ...
      events = ["one", "two"]
      add_bucket_events(events, ["bucket"], "client")
